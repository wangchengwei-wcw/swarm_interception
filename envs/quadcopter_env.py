...
from geometry_msgs.msg import TwistStamped, AccelStamped, Vector3Stamped, PointStamped
...
from utils.controller import Controller, bodyrate_control_without_thrust
...
class QuadcopterWaypointEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        logger.trace(f"Local trajectory generation takes {end - start:.6f}s")
    ...
    def _apply_action(self):
        if self.control_counter % self.cfg.control_decimation == 0:
            ...
            logger.trace(f"get_control takes {end - start:.6f}s")
            ...
            logger.trace(f"publish_debug_signals takes {end - start:.6f}s")
