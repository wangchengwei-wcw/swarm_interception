...
class SwarmAccEnvCfg(DirectMARLEnvCfg):
    ...
    # mutual_collision_avoidance_reward_weight = 6.0  # Stage 2
...
class SwarmAccEnv(DirectMARLEnv):
    ...
    def _reset_idx(self, env_ids: Sequence[int] | torch.Tensor | None):
        ...
        if len(mission_1_ids) > 0:
            r_max = self.cfg.flight_range - self.success_dist_thr[mission_1_ids][0] - 0.23
        ...
        if len(mission_2_ids) > 0:
            rg_max = self.cfg.flight_range - self.success_dist_thr[mission_2_ids][0] - 0.23
            if self.cfg.fix_range:
                # rg_max /= 1.25
                rg_min = rg_max
            else:
